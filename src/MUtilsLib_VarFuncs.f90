...
function checkPresent_str(var,valueIfNotPresent) result (checkPresent)
    ...
    if (present(var)) then
      If (len_trim(var)>len_vLongStr) call message($warn,"length of string var is greater than len_vLongStr:"//len_vLongStr//" - need to increase")
        checkPresent=trim(var)
     else
      If (len_trim(valueifNotPresent)>len_vLongStr) call message($warn,"length of string valueifNotPresent is greater than len_vLongStr:"//len_vLongStr//" - need to increase")

...
SUBROUTINE unitTest_init(name,nTests,nInputFiles,nRsltsFiles,nStndsFiles,err,&
   ...
   CHARACTER(LEN=*),INTENT(IN),DIMENSION(:),OPTIONAL::testNames      !> testNames      = [optional] Unit test names
   ...
   DO i=1,nTests
      ...
      IF(PRESENT(testNames))unitTest(i)%name=testNames(i)
  ...
  if (CompareLevelLc>1) then 
   ...
   DO
      ...
      IF((endOfFile_i==(-1)).and.(endOfFile_ii==(-1)))THEN;
       myTestResult=.TRUE.;CALL manageResult();EXIT;
...
SUBROUTINE unitTest_realTest(testVal,val_true,tol,err,message,outputUnit,testName)
   ...
   CHARACTER(LEN=*),INTENT(IN),OPTIONAL::testName,message
   !
   ! Local variables
   INTEGER(MIK)::indx
   !---
